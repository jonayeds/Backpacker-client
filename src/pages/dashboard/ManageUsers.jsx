  ...
  const handleSearch = (e) => {
    ...
    if (search) {
      setAllUsers(
        notFiltered.filter(
          (user) =>
            user.name.toLowerCase().split(" ").join("") === search ||
            user.email.toLowerCase().split(" ").join("") === search
        )
      );
    } else {
  ...
  return (
    <div>
      <div className="container p-2 mx-auto sm:p-4 dark:text-gray-800">
        <p className="text-4xl text-center mb-10 mt-12  logo text-[#283A2B] font-bold">
          Manage Users
        </p>
        <form
        ...
        >
          <input
            type="text"
            name="search"
            className="bg-[#d8dede] text-[#23575C] rounded-l-full py-2 px-4 focus:outline-none "
            placeholder="Search by Email or Name"
          />
          <button
            type="submit"
            className="py-2 pr-4 pl-2 rounded-r-full text-white bg-[#23575C]"
          >
            Search
          </button>
        </form>
        <div className="flex justify-center mb-20">
          <div className="dropdown">
            ...
            >
              <li>
                <a onClick={()=>setAllUsers(notFiltered)}>All</a>
              </li>
              <li>
                <a onClick={()=>setAllUsers(notFiltered.filter(user=> user.role === 'guide'))}>Guide</a>
              </li>
              <li>
                <a onClick={()=>setAllUsers(notFiltered.filter(user=> user.role === 'tourist'))}>Tourist</a>
              </li>
              <li>
                <a onClick={()=>setAllUsers(notFiltered.filter(user=> user.role === 'admin'))}>Admin</a>
              </li>
            </ul>
          </div>
        </div>
            ...
            <tbody>
              {allUsers.map((user) => (
                ...
                >
                  ...
                  <td className="p-3 text-center ">
                    <p
                      className={`max-w-max text-center mx-auto ${
                        user.requested === "Requested"
                          ? " bg-green-200 py-2 rounded-r-full pl-2 pr-4"
                          : ""
                      }  ${user.updated ? "hidden" : ""}`}
                    >
                      {user.requested === "Requested"
                  ...
                  <td className="p-3 text-center   text-white">
                    <button
                      ...
                      className={`bg-orange-400 py-2 px-4 rounded-sm ${
                        user.updated ? "hidden" : ""
                      }`}
                    >
                      {" "}
                  ...
                  <td className="p-3 text-center   text-white">
                    <button
                      ...
                      className={`bg-yellow-500 py-2 px-4 rounded-sm ${
                        user.updated ? "hidden" : ""
                      }`}
                    >
                      {" "}
